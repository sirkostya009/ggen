...
func (recv Node) decodeFromDepth(data []byte, depth int) (result Node, i int, err error) {
	...
	for {
		...
		case "extra":
			...
			seenExtra = true
			result.Extra, i, err = ggen.Any(data, i, true)
			if err != nil {
		...
		case "id":
			...
			result.ID = n
			if result.ID < 0 {
				return result, i, &ggen.GTEError{Pos: i, Path: []string{"id"}, Limit: int64(0), Value: result.ID}
			}
		case "matrix":
			if seenMatrix {
			...
			if i < len(data) && data[i] == 'n' {
				if i+4 > len(data) || data[i+1] != 'u' || data[i+2] != 'l' || data[i+3] != 'l' {
					return result, i, ggen.NewParseErr("matrix", i, ggen.ErrBadLiteral)
				}
				i += 4
				result.Matrix = nil
			} else {
		...
		case "parent":
			...
			if i < len(data) && data[i] == 'n' {
				if i+4 > len(data) || data[i+1] != 'u' || data[i+2] != 'l' || data[i+3] != 'l' {
					return result, i, ggen.NewParseErr("parent", i, ggen.ErrBadLiteral)
				}
				i += 4
				result.Parent = nil
				break
...
func (recv Node) decodeFromStreamDepth(s *ggen.Stream, depth int) (result Node, err error) {
	...
	for {
		...
		case "id":
			...
			if result.ID < 0 {
				return result, &ggen.GTEError{Pos: s.Offset(), Path: []string{"id"}, Limit: int64(0), Value: result.ID}
			}
		case "matrix":
			err = s.ConsumeColon()
			if err != nil {
...
func (recv MapValues) DecodeFrom(data []byte) (result MapValues, i int, err error) {
	...
	for {
		...
		case "entries":
			...
			if i < len(data) && data[i] == 'n' {
				if i+4 > len(data) || data[i+1] != 'u' || data[i+2] != 'l' || data[i+3] != 'l' {
					return result, i, ggen.NewParseErr("entries", i, ggen.ErrBadLiteral)
				}
				i += 4
				result.Entries = nil
				break
