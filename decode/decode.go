...
//
// The STREAMING side lives on the Stream itself — scan.StreamDecoder plus the
// generic methods (*scan.Stream).Value / .Slice:
//
//	res, _, err := T{}.DecodeFrom(data)
//	res, err := scan.NewStream(r, buf).Value[T]()
package decode
...
func UnmarshalSlice[T Decoder[T]](data []byte) ([]T, error) {
	...
	result := make([]T, 0, scan.PreallocCap(unsafe.Sizeof(*new(T))))
	for {
		var zero T
