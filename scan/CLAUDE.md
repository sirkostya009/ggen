...
- `SkipValue` skips numbers via `skipNumber` — a one-pass RFC 8259 grammar validator (`-?(0|[1-9][0-9]*)(\.[0-9]+)?([eE][+-]?[0-9]+)?`), NOT `Float64`/`ParseFloat`. Discarded numbers (RawMessage capture, `ignoreunknown`, `allowdups` value skips) only need the end position, so the float conversion is pure waste. Two accept-set shifts vs the old `Float64`-based skip, **both toward stdlib/jsonv2 parity**: (1) range-overflow grammar-valid numbers (`1e400`, `1e999`) now skip OK — a skip must not range-check; (2) ParseFloat-isms JSON forbids — leading `+` (`+1`), leading zero (`01`, `007`), bare/trailing dot (`.5`, `1.`) — are now rejected. Malformed numbers return `ErrBadNumber`, not `*strconv.NumError`. Measured **−3.04% `Mega_Unmarshal`** (p=0.000, n=12, core-pinned; Node's `Raw json.RawMessage` field is number-heavy) / **−1.10% `Mega_Reader/ggen_stream`** (stream `(*Stream).skipNumber`, copy/ReadMore-dominated so smaller), allocs/B byte-identical on both. Accept-set pinned by `TestSkipNumber_AcceptSetMatchesJSONGrammar` (differential vs `encoding/json.Valid`) + `TestSkipNumber_StreamMatchesBytes`; fuzz-clean (FuzzCompat/FuzzStreamEqualsBytes, ~20M execs). Actual number-VALUE fields (`float64`, `json.Number`) still use `Float64`/`Number` directly — untouched.
