...
func parseAnnotation(fset *token.FileSet, groups ...*ast.CommentGroup) (annotationFlags, bool, error) {
	for _, cg := range groups {
		...
		for _, c := range cg.List {
			...
			if rest == "" {
				return annotationFlags{}, true, nil
			}
	...
	return annotationFlags{}, false, nil
}
...
//
// The third return (siblings) is every type the package's per-file passes
// generate, used in single-file mode to seed generatedTypes so a cross-file
// reference routes to a direct DecodeFrom before sibling _ggen files exist.
// Nil in the AST-only degraded mode (siblings unknown).
func parseFile(filename string, wanted []string) ([]StructInfo, string, map[string]struct{}, map[string]struct{}, map[string]struct{}, map[string][]FieldInfo, error) {
	...
	if !degraded {
		...
		if allStructs, aerr := set.resolveFiltered(all, func(string) bool { return true }); aerr == nil {
			...
			pkgFields = seedGeneratedFields(allStructs)
		}
	}
...
// set; the returned name is the base package's.
func parsePackage(dir string) ([]StructInfo, string, error) {
	...
	if len(errs) > 0 {
		return nil, "", prefixBare(errors.Join(errs...), dir)
	}
	return structs, base.pkgName, nil
}
