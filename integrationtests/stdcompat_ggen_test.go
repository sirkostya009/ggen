...
func (recv F64Wire) DecodeFrom(data []byte) (result F64Wire, i int, err error) {
	...
	for {
		...
		case "v":
			...
			seenV = true
			result.V, i, err = scan.Float64(data, i)
			if err != nil {
...
func (s F64Wire) AppendJSON(dst []byte) ([]byte, error) {
	...
	_ = err
	dst = append(dst, "{\"v\":"...)
	if dst, err = encode.AppendFloat(dst, s.V, 64); err != nil {
		return dst, err
	}
	return append(dst, '}'), nil
}
...
func (recv F32Wire) DecodeFromStream(s *scan.Stream) (result F32Wire, err error) {
	...
	for {
		...
		case "v":
			...
			if math.IsInf(float64(float32(fv)), 0) {
				return result, decode.NewParseErr("v", s.Offset(), scan.ErrNumberOverflow)
			}
			result.V = float32(fv)
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
...
func (s F32Wire) AppendJSON(dst []byte) ([]byte, error) {
	...
	_ = err
	dst = append(dst, "{\"v\":"...)
	if dst, err = encode.AppendFloat(dst, float64(s.V), 32); err != nil {
		return dst, err
	}
	return append(dst, '}'), nil
}
...
func (recv AnyWire) DecodeFrom(data []byte) (result AnyWire, i int, err error) {
	...
	for {
		...
		case "v":
			...
			seenV = true
			result.V, i, err = scan.Any(data, i)
			if err != nil {
				return result, i, decode.NewParseErr("v", i, err)
			}
		default:
...
func (s AnyWire) AppendJSON(dst []byte) ([]byte, error) {
	...
	_ = err
	dst = append(dst, "{\"v\":"...)
	if dst, err = encode.AppendAny(dst, s.V); err != nil {
		return dst, err
	}
	return append(dst, '}'), nil
}
