...
func (recv CopyRef) DecodeFrom(data []byte) (result CopyRef, i int, err error) {
	...
	for {
		...
		case "label":
			...
			if ke >= len(data) {
				return result, i, decode.NewParseErr("label", i, scan.ErrUnterminated)
			}
			if data[ke] < 0x20 {
				return result, i, decode.NewParseErr("label", i, scan.ErrBadString)
			}
			if data[ke] == '"' {
...
func (recv CopyRef) DecodeFromStream(s *scan.Stream) (result CopyRef, err error) {
	...
	for {
		...
		case "label":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("label", s.Pos, err)
			}
			if seenLabel {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"label"}}
			}
			seenLabel = true
			result.Label, err = s.String()
			if err != nil {
				return result, decode.NewParseErr("label", s.Pos, err)
			}
		default:
