...
| `pointer_test.go`       | `*T` — null ↔ nil, primitive + struct pointees, `[]*T` slab. Single-level fields split per-pointee-kind (`PtrName`/`PtrCount`/… embedded by composite `PointerStruct`); `NPtrStruct` embeds per-depth `**T`/`***T`/`****T`/`**struct` structs, all native (no json fallback). Per-field JSONSize cap-guards in `jsonsize_test.go` (`PtrFieldPerKind`/`NPtrPerDepth`). Slice/array `[]**T` variants gated on n-pointer backlog item. |
