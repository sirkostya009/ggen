...
- `FuzzPrimitivesCompat` — fuzzes typed VALUES, not payload bytes. `PrimStruct` carries one field per primitive kind (bool, every int/uint width, float32/64, string); fuzzed values are stdlib-marshaled into a well-formed payload, then ggen and jsonv2 must decode it identically (both accept/reject, equal structs on success). Drives value-parsers across their full domain. Skips invalid-UTF-8 strings and NaN/Inf floats (no shared JSON form). `float32` decodes via `scan.Float64`+cast (double-round) vs stdlib's bitSize 32 — clean because jsonv2 emits shortest-round-tripping decimals.
