...
| `maps_test.go`          | String-keyed maps; key validators / mods; deep map values. `NestedMaps`: map-of-map at every value shape — each nesting level names its own key/value locals, bytes + stream. `NamedValMaps`: named-primitive map values marshal via the primCast'd ref (round-trip).                                                                                                                                                                                                                                                                                                                                                                                                                         |
