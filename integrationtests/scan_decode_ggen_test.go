// Code generated by ggen; DO NOT EDIT.

package integrationtests

import (
	"bytes"
	"encoding/json"
	"math"
	"strconv"
	"strings"
	"unsafe"

	"github.com/sirkostya009/ggen/decode"
	"github.com/sirkostya009/ggen/encode"
	"github.com/sirkostya009/ggen/scan"
	"github.com/sirkostya009/ggen/validation"
)

// Tries to fit >2 elements in 80 bytes, then 512 bytes - never goes above that.
const ggenCap_NarrowInts_SliI_int16 = (min((80/max(int(unsafe.Sizeof(*new(int16))), 1)), 2)/2)*(80/max(int(unsafe.Sizeof(*new(int16))), 1)) + (1-(min((80/max(int(unsafe.Sizeof(*new(int16))), 1)), 2)/2))*max(((8*int(unsafe.Sizeof(uintptr(0)))*int(unsafe.Sizeof(uintptr(0))))/max(int(unsafe.Sizeof(*new(int16))), 1)), 1)

// Tries to fit >2 elements in 80 bytes, then 512 bytes - never goes above that.
const ggenCap_NarrowFloats_Fs_float32 = (min((80/max(int(unsafe.Sizeof(*new(float32))), 1)), 2)/2)*(80/max(int(unsafe.Sizeof(*new(float32))), 1)) + (1-(min((80/max(int(unsafe.Sizeof(*new(float32))), 1)), 2)/2))*max(((8*int(unsafe.Sizeof(uintptr(0)))*int(unsafe.Sizeof(uintptr(0))))/max(int(unsafe.Sizeof(*new(float32))), 1)), 1)

func (recv HugeStringStruct) DecodeFrom(data []byte) (result HugeStringStruct, i int, err error) {
	result = recv
	seenBig := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "big":
			if seenBig {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"big"}}
			}
			seenBig = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("big", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.Big = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.Big, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("big", i, err)
				}
			}
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv HugeStringStruct) DecodeFromStream(s *scan.Stream) (result HugeStringStruct, err error) {
	result = recv
	seenBig := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "big":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("big", s.Offset(), err)
			}
			if seenBig {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"big"}}
			}
			seenBig = true
			result.Big, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("big", s.Offset(), err)
			}
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s HugeStringStruct) JSONSize() int {
	size := 10
	size += len(s.Big) * 2
	return size
}

func (s HugeStringStruct) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"big\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.Big)
	return append(dst, '}'), nil
}

func (recv SequentialStringsStruct) DecodeFrom(data []byte) (result SequentialStringsStruct, i int, err error) {
	result = recv
	seenA := false
	seenB := false
	seenC := false
	seenD := false
	seenE := false
	seenF := false
	seenG := false
	seenH := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "a":
			if seenA {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"a"}}
			}
			seenA = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("a", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.A = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.A, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("a", i, err)
				}
			}
		case "b":
			if seenB {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"b"}}
			}
			seenB = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("b", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.B = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.B, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("b", i, err)
				}
			}
		case "c":
			if seenC {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"c"}}
			}
			seenC = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("c", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.C = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.C, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("c", i, err)
				}
			}
		case "d":
			if seenD {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"d"}}
			}
			seenD = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("d", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.D = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.D, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("d", i, err)
				}
			}
		case "e":
			if seenE {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"e"}}
			}
			seenE = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("e", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.E = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.E, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("e", i, err)
				}
			}
		case "f":
			if seenF {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"f"}}
			}
			seenF = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("f", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.F = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.F, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("f", i, err)
				}
			}
		case "g":
			if seenG {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"g"}}
			}
			seenG = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("g", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.G = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.G, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("g", i, err)
				}
			}
		case "h":
			if seenH {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"h"}}
			}
			seenH = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("h", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.H = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.H, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("h", i, err)
				}
			}
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv SequentialStringsStruct) DecodeFromStream(s *scan.Stream) (result SequentialStringsStruct, err error) {
	result = recv
	seenA := false
	seenB := false
	seenC := false
	seenD := false
	seenE := false
	seenF := false
	seenG := false
	seenH := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "a":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("a", s.Offset(), err)
			}
			if seenA {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"a"}}
			}
			seenA = true
			result.A, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("a", s.Offset(), err)
			}
		case "b":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("b", s.Offset(), err)
			}
			if seenB {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"b"}}
			}
			seenB = true
			result.B, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("b", s.Offset(), err)
			}
		case "c":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("c", s.Offset(), err)
			}
			if seenC {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"c"}}
			}
			seenC = true
			result.C, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("c", s.Offset(), err)
			}
		case "d":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("d", s.Offset(), err)
			}
			if seenD {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"d"}}
			}
			seenD = true
			result.D, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("d", s.Offset(), err)
			}
		case "e":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("e", s.Offset(), err)
			}
			if seenE {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"e"}}
			}
			seenE = true
			result.E, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("e", s.Offset(), err)
			}
		case "f":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("f", s.Offset(), err)
			}
			if seenF {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"f"}}
			}
			seenF = true
			result.F, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("f", s.Offset(), err)
			}
		case "g":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("g", s.Offset(), err)
			}
			if seenG {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"g"}}
			}
			seenG = true
			result.G, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("g", s.Offset(), err)
			}
		case "h":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("h", s.Offset(), err)
			}
			if seenH {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"h"}}
			}
			seenH = true
			result.H, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("h", s.Offset(), err)
			}
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s SequentialStringsStruct) JSONSize() int {
	size := 57
	size += len(s.A) * 2
	size += len(s.B) * 2
	size += len(s.C) * 2
	size += len(s.D) * 2
	size += len(s.E) * 2
	size += len(s.F) * 2
	size += len(s.G) * 2
	size += len(s.H) * 2
	return size
}

func (s SequentialStringsStruct) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"a\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.A)
	dst = append(dst, ",\"b\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.B)
	dst = append(dst, ",\"c\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.C)
	dst = append(dst, ",\"d\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.D)
	dst = append(dst, ",\"e\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.E)
	dst = append(dst, ",\"f\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.F)
	dst = append(dst, ",\"g\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.G)
	dst = append(dst, ",\"h\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.H)
	return append(dst, '}'), nil
}

func (recv UnknownErrorStruct) DecodeFrom(data []byte) (result UnknownErrorStruct, i int, err error) {
	result = recv
	seenName := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "name":
			if seenName {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"name"}}
			}
			seenName = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("name", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.Name = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.Name, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("name", i, err)
				}
			}
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv UnknownErrorStruct) DecodeFromStream(s *scan.Stream) (result UnknownErrorStruct, err error) {
	result = recv
	seenName := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "name":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("name", s.Offset(), err)
			}
			if seenName {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"name"}}
			}
			seenName = true
			result.Name, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("name", s.Offset(), err)
			}
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s UnknownErrorStruct) JSONSize() int {
	size := 11
	size += len(s.Name) * 2
	return size
}

func (s UnknownErrorStruct) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"name\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.Name)
	return append(dst, '}'), nil
}

func (recv ValidationPosStruct) DecodeFrom(data []byte) (result ValidationPosStruct, i int, err error) {
	result = recv
	seenA := false
	seenB := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "a":
			if seenA {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"a"}}
			}
			seenA = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("a", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.A = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.A, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("a", i, err)
				}
			}
		case "b":
			if seenB {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"b"}}
			}
			seenB = true
			if i >= len(data) || data[i] != '"' {
				return result, i, decode.NewParseErr("b", i, scan.ErrExpectString)
			}
			ke := i + 1
			kew := ke + 32
			if kew > len(data) {
				kew = len(data)
			}
			for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
				ke++
			}
			if ke < len(data) && data[ke] == '"' {
				result.B = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
				i = ke + 1
			} else {
				result.B, i, err = scan.String(data, i, true)
				if err != nil {
					return result, i, decode.NewParseErr("b", i, err)
				}
			}
			if len(result.B) < 4 {
				return result, i, &validation.MinLenError{Pos: i, Path: []string{"b"}, Limit: 4, Got: len(result.B)}
			}
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv ValidationPosStruct) DecodeFromStream(s *scan.Stream) (result ValidationPosStruct, err error) {
	result = recv
	seenA := false
	seenB := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "a":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("a", s.Offset(), err)
			}
			if seenA {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"a"}}
			}
			seenA = true
			result.A, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("a", s.Offset(), err)
			}
		case "b":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("b", s.Offset(), err)
			}
			if seenB {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"b"}}
			}
			seenB = true
			result.B, err = s.String(true)
			if err != nil {
				return result, decode.NewParseErr("b", s.Offset(), err)
			}
			if len(result.B) < 4 {
				return result, &validation.MinLenError{Pos: s.Offset(), Path: []string{"b"}, Limit: 4, Got: len(result.B)}
			}
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s ValidationPosStruct) JSONSize() int {
	size := 15
	size += len(s.A) * 2
	size += len(s.B) * 2
	return size
}

func (s ValidationPosStruct) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"a\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.A)
	dst = append(dst, ",\"b\":\""...)
	dst = encode.AppendStringNoHTML(dst, s.B)
	return append(dst, '}'), nil
}

func (recv NarrowInts) DecodeFrom(data []byte) (result NarrowInts, i int, err error) {
	result = recv
	if result.MapU != nil {
		clear(result.MapU)
	}
	if result.SliI != nil {
		result.SliI = result.SliI[:0]
	}
	seenI16 := false
	seenI32 := false
	seenI8 := false
	seenMapU := false
	seenPtrU := false
	seenSliI := false
	seenU16 := false
	seenU32 := false
	seenU8 := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "i16":
			if seenI16 {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"i16"}}
			}
			seenI16 = true
			neg := false
			if i < len(data) && data[i] == '-' {
				neg = true
				i++
			}
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("i16", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("i16", i, scan.ErrBadNumber)
			}
			limit := uint64(math.MaxInt64)
			if neg {
				limit = scan.SignedNeg
			}
			var u uint64
			de := i + 18
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				u = u*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if u > limit/10 || (u == limit/10 && d > limit%10) {
					return result, i, decode.NewParseErr("i16", i, scan.ErrNumberOverflow)
				}
				u = u*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("i16", i, scan.ErrBadNumber)
				}
			}
			var n int64
			if neg {
				if u == scan.SignedNeg {
					n = math.MinInt64
				} else {
					n = -int64(u)
				}
			} else {
				n = int64(u)
			}
			if n < math.MinInt16 || n > math.MaxInt16 {
				return result, i, decode.NewParseErr("i16", i, scan.ErrNumberOverflow)
			}
			result.I16 = int16(n)
		case "i32":
			if seenI32 {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"i32"}}
			}
			seenI32 = true
			neg := false
			if i < len(data) && data[i] == '-' {
				neg = true
				i++
			}
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("i32", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("i32", i, scan.ErrBadNumber)
			}
			limit := uint64(math.MaxInt64)
			if neg {
				limit = scan.SignedNeg
			}
			var u uint64
			de := i + 18
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				u = u*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if u > limit/10 || (u == limit/10 && d > limit%10) {
					return result, i, decode.NewParseErr("i32", i, scan.ErrNumberOverflow)
				}
				u = u*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("i32", i, scan.ErrBadNumber)
				}
			}
			var n int64
			if neg {
				if u == scan.SignedNeg {
					n = math.MinInt64
				} else {
					n = -int64(u)
				}
			} else {
				n = int64(u)
			}
			if n < math.MinInt32 || n > math.MaxInt32 {
				return result, i, decode.NewParseErr("i32", i, scan.ErrNumberOverflow)
			}
			result.I32 = int32(n)
		case "i8":
			if seenI8 {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"i8"}}
			}
			seenI8 = true
			neg := false
			if i < len(data) && data[i] == '-' {
				neg = true
				i++
			}
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("i8", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("i8", i, scan.ErrBadNumber)
			}
			limit := uint64(math.MaxInt64)
			if neg {
				limit = scan.SignedNeg
			}
			var u uint64
			de := i + 18
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				u = u*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if u > limit/10 || (u == limit/10 && d > limit%10) {
					return result, i, decode.NewParseErr("i8", i, scan.ErrNumberOverflow)
				}
				u = u*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("i8", i, scan.ErrBadNumber)
				}
			}
			var n int64
			if neg {
				if u == scan.SignedNeg {
					n = math.MinInt64
				} else {
					n = -int64(u)
				}
			} else {
				n = int64(u)
			}
			if n < math.MinInt8 || n > math.MaxInt8 {
				return result, i, decode.NewParseErr("i8", i, scan.ErrNumberOverflow)
			}
			result.I8 = int8(n)
		case "mapU":
			if seenMapU {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"mapU"}}
			}
			seenMapU = true
			if i+4 <= len(data) && data[i] == 'n' && data[i+1] == 'u' && data[i+2] == 'l' && data[i+3] == 'l' {
				i += 4
				result.MapU = nil
				break
			}
			if i >= len(data) || data[i] != '{' {
				return result, i, decode.NewParseErr("mapU", i, scan.ErrBadObject)
			}
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			if i < len(data) && data[i] == '}' {
				if result.MapU == nil {
					result.MapU = map[string]uint8{}
				}
			} else {
				if result.MapU == nil {
					result.MapU = make(map[string]uint8)
				}
			}
			if i < len(data) && data[i] != '}' {
				for {
					var mk string
					if i >= len(data) || data[i] != '"' {
						return result, i, decode.NewParseErr("mapU", i, scan.ErrExpectString)
					}
					ke := i + 1
					kew := ke + 32
					if kew > len(data) {
						kew = len(data)
					}
					for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
						ke++
					}
					if ke < len(data) && data[ke] == '"' {
						mk = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
						i = ke + 1
					} else {
						mk, i, err = scan.String(data, i, true)
						if err != nil {
							return result, i, decode.NewParseErr("mapU", i, err)
						}
					}
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i >= len(data) || data[i] != ':' {
						return result, i, decode.NewParseErr("mapU", i, scan.ErrBadObject)
					}
					i++
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i >= len(data) || data[i] < '0' || data[i] > '9' {
						return result, i, decode.NewParseErr("mapU", i, scan.ErrBadNumber)
					}
					if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
						return result, i, decode.NewParseErr("mapU", i, scan.ErrBadNumber)
					}
					var n uint64
					de := i + 19
					if de > len(data) {
						de = len(data)
					}
					for i < de && data[i] >= '0' && data[i] <= '9' {
						n = n*10 + uint64(data[i]-'0')
						i++
					}
					for i < len(data) && data[i] >= '0' && data[i] <= '9' {
						d := uint64(data[i] - '0')
						if n > scan.Uint64Limit/10 || (n == scan.Uint64Limit/10 && d > scan.Uint64Limit%10) {
							return result, i, decode.NewParseErr("mapU", i, scan.ErrNumberOverflow)
						}
						n = n*10 + d
						i++
					}
					if i < len(data) {
						c := data[i]
						if c == '.' || c == 'e' || c == 'E' {
							return result, i, decode.NewParseErr("mapU", i, scan.ErrBadNumber)
						}
					}
					if n > math.MaxUint8 {
						return result, i, decode.NewParseErr("mapU", i, scan.ErrNumberOverflow)
					}
					result.MapU[mk] = uint8(n)
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i < len(data) && data[i] == ',' {
						i++
						for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
							i++
						}
						if i >= len(data) || data[i] == '}' {
							return result, i, decode.NewParseErr("mapU", i, scan.ErrBadObject)
						}
						continue
					}
					break
				}
			}
			if i >= len(data) || data[i] != '}' {
				return result, i, decode.NewParseErr("mapU", i, scan.ErrBadObject)
			}
			i++
		case "ptrU":
			if seenPtrU {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"ptrU"}}
			}
			seenPtrU = true
			if i+4 <= len(data) && data[i] == 'n' && data[i+1] == 'u' && data[i+2] == 'l' && data[i+3] == 'l' {
				i += 4
				result.PtrU = nil
				break
			}
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("ptrU", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("ptrU", i, scan.ErrBadNumber)
			}
			var n uint64
			de := i + 19
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				n = n*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if n > scan.Uint64Limit/10 || (n == scan.Uint64Limit/10 && d > scan.Uint64Limit%10) {
					return result, i, decode.NewParseErr("ptrU", i, scan.ErrNumberOverflow)
				}
				n = n*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("ptrU", i, scan.ErrBadNumber)
				}
			}
			if n > math.MaxUint8 {
				return result, i, decode.NewParseErr("ptrU", i, scan.ErrNumberOverflow)
			}
			if result.PtrU == nil {
				result.PtrU = new(uint8(n))
			} else {
				(*result.PtrU) = uint8(n)
			}
		case "sliI":
			if seenSliI {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"sliI"}}
			}
			seenSliI = true
			if i+4 <= len(data) && data[i] == 'n' && data[i+1] == 'u' && data[i+2] == 'l' && data[i+3] == 'l' {
				i += 4
				result.SliI = nil
				break
			}
			if i >= len(data) || data[i] != '[' {
				return result, i, decode.NewParseErr("sliI", i, scan.ErrBadArray)
			}
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			if i < len(data) && data[i] == ']' {
				if result.SliI == nil {
					result.SliI = []int16{}
				}
			} else {
				if result.SliI == nil {
					cnt0 := 1
					if e := bytes.IndexByte(data[i:], ']'); e >= 0 {
						cnt0 = bytes.Count(data[i:i+e], []byte{','}) + 1
					}
					result.SliI = make([]int16, 0, cnt0)
				}
			}
			if i < len(data) && data[i] != ']' {
				for {
					result.SliI = append(result.SliI, 0)
					neg := false
					if i < len(data) && data[i] == '-' {
						neg = true
						i++
					}
					if i >= len(data) || data[i] < '0' || data[i] > '9' {
						return result, i, decode.NewParseErr("sliI", i, scan.ErrBadNumber)
					}
					if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
						return result, i, decode.NewParseErr("sliI", i, scan.ErrBadNumber)
					}
					limit := uint64(math.MaxInt64)
					if neg {
						limit = scan.SignedNeg
					}
					var u uint64
					de := i + 18
					if de > len(data) {
						de = len(data)
					}
					for i < de && data[i] >= '0' && data[i] <= '9' {
						u = u*10 + uint64(data[i]-'0')
						i++
					}
					for i < len(data) && data[i] >= '0' && data[i] <= '9' {
						d := uint64(data[i] - '0')
						if u > limit/10 || (u == limit/10 && d > limit%10) {
							return result, i, decode.NewParseErr("sliI", i, scan.ErrNumberOverflow)
						}
						u = u*10 + d
						i++
					}
					if i < len(data) {
						c := data[i]
						if c == '.' || c == 'e' || c == 'E' {
							return result, i, decode.NewParseErr("sliI", i, scan.ErrBadNumber)
						}
					}
					var n int64
					if neg {
						if u == scan.SignedNeg {
							n = math.MinInt64
						} else {
							n = -int64(u)
						}
					} else {
						n = int64(u)
					}
					if n < math.MinInt16 || n > math.MaxInt16 {
						return result, i, decode.NewParseErr("sliI", i, scan.ErrNumberOverflow)
					}
					result.SliI[len(result.SliI)-1] = int16(n)
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i < len(data) && data[i] == ',' {
						i++
						for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
							i++
						}
						if i >= len(data) || data[i] == ']' {
							return result, i, decode.NewParseErr("sliI", i, scan.ErrBadArray)
						}
						continue
					}
					break
				}
			}
			if i >= len(data) || data[i] != ']' {
				return result, i, decode.NewParseErr("sliI", i, scan.ErrBadArray)
			}
			i++
		case "u16":
			if seenU16 {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"u16"}}
			}
			seenU16 = true
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("u16", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("u16", i, scan.ErrBadNumber)
			}
			var n uint64
			de := i + 19
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				n = n*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if n > scan.Uint64Limit/10 || (n == scan.Uint64Limit/10 && d > scan.Uint64Limit%10) {
					return result, i, decode.NewParseErr("u16", i, scan.ErrNumberOverflow)
				}
				n = n*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("u16", i, scan.ErrBadNumber)
				}
			}
			if n > math.MaxUint16 {
				return result, i, decode.NewParseErr("u16", i, scan.ErrNumberOverflow)
			}
			result.U16 = uint16(n)
		case "u32":
			if seenU32 {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"u32"}}
			}
			seenU32 = true
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("u32", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("u32", i, scan.ErrBadNumber)
			}
			var n uint64
			de := i + 19
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				n = n*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if n > scan.Uint64Limit/10 || (n == scan.Uint64Limit/10 && d > scan.Uint64Limit%10) {
					return result, i, decode.NewParseErr("u32", i, scan.ErrNumberOverflow)
				}
				n = n*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("u32", i, scan.ErrBadNumber)
				}
			}
			if n > math.MaxUint32 {
				return result, i, decode.NewParseErr("u32", i, scan.ErrNumberOverflow)
			}
			result.U32 = uint32(n)
		case "u8":
			if seenU8 {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"u8"}}
			}
			seenU8 = true
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("u8", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("u8", i, scan.ErrBadNumber)
			}
			var n uint64
			de := i + 19
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				n = n*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if n > scan.Uint64Limit/10 || (n == scan.Uint64Limit/10 && d > scan.Uint64Limit%10) {
					return result, i, decode.NewParseErr("u8", i, scan.ErrNumberOverflow)
				}
				n = n*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("u8", i, scan.ErrBadNumber)
				}
			}
			if n > math.MaxUint8 {
				return result, i, decode.NewParseErr("u8", i, scan.ErrNumberOverflow)
			}
			result.U8 = uint8(n)
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv NarrowInts) DecodeFromStream(s *scan.Stream) (result NarrowInts, err error) {
	result = recv
	if result.MapU != nil {
		clear(result.MapU)
	}
	if result.SliI != nil {
		result.SliI = result.SliI[:0]
	}
	seenI16 := false
	seenI32 := false
	seenI8 := false
	seenMapU := false
	seenPtrU := false
	seenSliI := false
	seenU16 := false
	seenU32 := false
	seenU8 := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "i16":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("i16", s.Offset(), err)
			}
			if seenI16 {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"i16"}}
			}
			seenI16 = true
			var iv int64
			iv, err = s.Int64()
			if err != nil {
				return result, decode.NewParseErr("i16", s.Offset(), err)
			}
			if iv < math.MinInt16 || iv > math.MaxInt16 {
				return result, decode.NewParseErr("i16", s.Offset(), scan.ErrNumberOverflow)
			}
			result.I16 = int16(iv)
		case "i32":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("i32", s.Offset(), err)
			}
			if seenI32 {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"i32"}}
			}
			seenI32 = true
			var iv int64
			iv, err = s.Int64()
			if err != nil {
				return result, decode.NewParseErr("i32", s.Offset(), err)
			}
			if iv < math.MinInt32 || iv > math.MaxInt32 {
				return result, decode.NewParseErr("i32", s.Offset(), scan.ErrNumberOverflow)
			}
			result.I32 = int32(iv)
		case "i8":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("i8", s.Offset(), err)
			}
			if seenI8 {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"i8"}}
			}
			seenI8 = true
			var iv int64
			iv, err = s.Int64()
			if err != nil {
				return result, decode.NewParseErr("i8", s.Offset(), err)
			}
			if iv < math.MinInt8 || iv > math.MaxInt8 {
				return result, decode.NewParseErr("i8", s.Offset(), scan.ErrNumberOverflow)
			}
			result.I8 = int8(iv)
		case "mapU":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("mapU", s.Offset(), err)
			}
			if seenMapU {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"mapU"}}
			}
			seenMapU = true
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("mapU", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
				}
			}
			if s.Bytes()[s.Pos] == 'n' {
				for ki := 1; ki < 4; ki++ {
					if s.Pos+ki >= len(s.Bytes()) {
						if err = s.ReadMore(0); err != nil {
							return result, decode.NewParseErr("mapU", s.Offset(), scan.NotEOF(err, scan.ErrBadLiteral))
						}
					}
					if s.Bytes()[s.Pos+ki] != "null"[ki] {
						return result, decode.NewParseErr("mapU", s.Offset(), scan.ErrBadLiteral)
					}
				}
				s.Pos += 4
				result.MapU = nil
				break
			}
			err = s.ObjectOpen()
			if err != nil {
				return result, decode.NewParseErr("mapU", s.Offset(), err)
			}
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("mapU", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
				}
			}
			if s.Bytes()[s.Pos] == '}' {
				if result.MapU == nil {
					result.MapU = map[string]uint8{}
				}
			} else {
				if result.MapU == nil {
					result.MapU = make(map[string]uint8)
				}
			}
			for s.Bytes()[s.Pos] != '}' {
				var mk string
				mk, err = s.String(true)
				if err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), err)
				}
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), err)
				}
				if s.Pos >= len(s.Bytes()) {
					if err = s.ReadMore(0); err != nil {
						return result, decode.NewParseErr("mapU", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
					}
				}
				if s.Bytes()[s.Pos] != ':' {
					return result, decode.NewParseErr("mapU", s.Offset(), scan.ErrBadObject)
				}
				s.Pos++
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), err)
				}
				var uv uint64
				uv, err = s.Uint64()
				if err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), err)
				}
				if uv > math.MaxUint8 {
					return result, decode.NewParseErr("mapU", s.Offset(), scan.ErrNumberOverflow)
				}
				result.MapU[mk] = uint8(uv)
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("mapU", s.Offset(), err)
				}
				if s.Pos >= len(s.Bytes()) {
					if err = s.ReadMore(0); err != nil {
						return result, decode.NewParseErr("mapU", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
					}
				}
				if s.Bytes()[s.Pos] == ',' {
					s.Pos++
					err = s.SkipSpace()
					if err != nil {
						return result, decode.NewParseErr("mapU", s.Offset(), err)
					}
					if s.Pos >= len(s.Bytes()) || s.Bytes()[s.Pos] == '}' {
						return result, decode.NewParseErr("mapU", s.Offset(), scan.ErrBadObject)
					}
					continue
				}
				break
			}
			if s.Bytes()[s.Pos] != '}' {
				return result, decode.NewParseErr("mapU", s.Offset(), scan.ErrBadObject)
			}
			s.Pos++
		case "ptrU":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("ptrU", s.Offset(), err)
			}
			if seenPtrU {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"ptrU"}}
			}
			seenPtrU = true
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("ptrU", s.Offset(), scan.NotEOF(err, scan.ErrBadNumber))
				}
			}
			if s.Bytes()[s.Pos] == 'n' {
				for ki := 1; ki < 4; ki++ {
					if s.Pos+ki >= len(s.Bytes()) {
						if err = s.ReadMore(0); err != nil {
							return result, decode.NewParseErr("ptrU", s.Offset(), scan.NotEOF(err, scan.ErrBadLiteral))
						}
					}
					if s.Bytes()[s.Pos+ki] != "null"[ki] {
						return result, decode.NewParseErr("ptrU", s.Offset(), scan.ErrBadLiteral)
					}
				}
				s.Pos += 4
				result.PtrU = nil
				break
			}
			var v uint64
			v, err = s.Uint64()
			if err != nil {
				return result, decode.NewParseErr("ptrU", s.Offset(), err)
			}
			if v > math.MaxUint8 {
				return result, decode.NewParseErr("ptrU", s.Offset(), scan.ErrNumberOverflow)
			}
			if result.PtrU == nil {
				result.PtrU = new(uint8(v))
			} else {
				(*result.PtrU) = uint8(v)
			}
		case "sliI":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("sliI", s.Offset(), err)
			}
			if seenSliI {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"sliI"}}
			}
			seenSliI = true
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("sliI", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("sliI", s.Offset(), scan.NotEOF(err, scan.ErrBadArray))
				}
			}
			if s.Bytes()[s.Pos] == 'n' {
				for ki := 1; ki < 4; ki++ {
					if s.Pos+ki >= len(s.Bytes()) {
						if err = s.ReadMore(0); err != nil {
							return result, decode.NewParseErr("sliI", s.Offset(), scan.NotEOF(err, scan.ErrBadLiteral))
						}
					}
					if s.Bytes()[s.Pos+ki] != "null"[ki] {
						return result, decode.NewParseErr("sliI", s.Offset(), scan.ErrBadLiteral)
					}
				}
				s.Pos += 4
				result.SliI = nil
				break
			}
			err = s.ArrayOpen()
			if err != nil {
				return result, decode.NewParseErr("sliI", s.Offset(), err)
			}
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("sliI", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("sliI", s.Offset(), scan.NotEOF(err, scan.ErrBadArray))
				}
			}
			if s.Bytes()[s.Pos] == ']' {
				if result.SliI == nil {
					result.SliI = []int16{}
				}
			} else {
				if result.SliI == nil {
					result.SliI = make([]int16, 0, ggenCap_NarrowInts_SliI_int16)
				}
			}
			for s.Bytes()[s.Pos] != ']' {
				result.SliI = append(result.SliI, 0)
				var iv int64
				iv, err = s.Int64()
				if err != nil {
					return result, decode.NewParseErr("sliI", s.Offset(), err)
				}
				if iv < math.MinInt16 || iv > math.MaxInt16 {
					return result, decode.NewParseErr("sliI", s.Offset(), scan.ErrNumberOverflow)
				}
				result.SliI[len(result.SliI)-1] = int16(iv)
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("sliI", s.Offset(), err)
				}
				if s.Pos >= len(s.Bytes()) {
					if err = s.ReadMore(0); err != nil {
						return result, decode.NewParseErr("sliI", s.Offset(), scan.NotEOF(err, scan.ErrBadArray))
					}
				}
				if s.Bytes()[s.Pos] == ',' {
					s.Pos++
					err = s.SkipSpace()
					if err != nil {
						return result, decode.NewParseErr("sliI", s.Offset(), err)
					}
					if s.Pos >= len(s.Bytes()) || s.Bytes()[s.Pos] == ']' {
						return result, decode.NewParseErr("sliI", s.Offset(), scan.ErrBadArray)
					}
					continue
				}
				break
			}
			if s.Bytes()[s.Pos] != ']' {
				return result, decode.NewParseErr("sliI", s.Offset(), scan.ErrBadArray)
			}
			s.Pos++
		case "u16":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("u16", s.Offset(), err)
			}
			if seenU16 {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"u16"}}
			}
			seenU16 = true
			var uv uint64
			uv, err = s.Uint64()
			if err != nil {
				return result, decode.NewParseErr("u16", s.Offset(), err)
			}
			if uv > math.MaxUint16 {
				return result, decode.NewParseErr("u16", s.Offset(), scan.ErrNumberOverflow)
			}
			result.U16 = uint16(uv)
		case "u32":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("u32", s.Offset(), err)
			}
			if seenU32 {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"u32"}}
			}
			seenU32 = true
			var uv uint64
			uv, err = s.Uint64()
			if err != nil {
				return result, decode.NewParseErr("u32", s.Offset(), err)
			}
			if uv > math.MaxUint32 {
				return result, decode.NewParseErr("u32", s.Offset(), scan.ErrNumberOverflow)
			}
			result.U32 = uint32(uv)
		case "u8":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("u8", s.Offset(), err)
			}
			if seenU8 {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"u8"}}
			}
			seenU8 = true
			var uv uint64
			uv, err = s.Uint64()
			if err != nil {
				return result, decode.NewParseErr("u8", s.Offset(), err)
			}
			if uv > math.MaxUint8 {
				return result, decode.NewParseErr("u8", s.Offset(), scan.ErrNumberOverflow)
			}
			result.U8 = uint8(uv)
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s NarrowInts) JSONSize() int {
	size := 193
	size += len(s.MapU) * 24
	for k := range s.MapU {
		size += len(k) * 2
	}
	if s.PtrU == nil {
		size += 4
	} else {
		size += 20
	}
	if n := len(s.SliI); n > 0 {
		size += n - 1
	}
	size += len(s.SliI) * 20
	return size
}

func (s NarrowInts) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"i16\":"...)
	dst = strconv.AppendInt(dst, int64(s.I16), 10)
	dst = append(dst, ",\"i32\":"...)
	dst = strconv.AppendInt(dst, int64(s.I32), 10)
	dst = append(dst, ",\"i8\":"...)
	dst = strconv.AppendInt(dst, int64(s.I8), 10)
	dst = append(dst, ",\"mapU\":"...)
	if s.MapU == nil {
		dst = append(dst, "null"...)
	} else {
		dst = append(dst, '{')
		firstMapU := true
		for k, v := range s.MapU {
			if firstMapU {
				firstMapU = false
				dst = append(dst, '"')
			} else {
				dst = append(dst, ",\""...)
			}
			dst = encode.AppendStringNoHTML(dst, k)
			dst = append(dst, ':')
			dst = strconv.AppendUint(dst, uint64(v), 10)
		}
		dst = append(dst, '}')
	}
	dst = append(dst, ",\"ptrU\":"...)
	if s.PtrU == nil {
		dst = append(dst, "null"...)
	} else {
		dst = strconv.AppendUint(dst, uint64((*s.PtrU)), 10)
	}
	dst = append(dst, ",\"sliI\":"...)
	if s.SliI == nil {
		dst = append(dst, "null"...)
	} else {
		dst = append(dst, '[')
		if len(s.SliI) > 0 {
			dst = strconv.AppendInt(dst, int64(s.SliI[0]), 10)
			for _, v0 := range s.SliI[1:] {
				dst = append(dst, ',')
				dst = strconv.AppendInt(dst, int64(v0), 10)
			}
		}
		dst = append(dst, ']')
	}
	dst = append(dst, ",\"u16\":"...)
	dst = strconv.AppendUint(dst, uint64(s.U16), 10)
	dst = append(dst, ",\"u32\":"...)
	dst = strconv.AppendUint(dst, uint64(s.U32), 10)
	dst = append(dst, ",\"u8\":"...)
	dst = strconv.AppendUint(dst, uint64(s.U8), 10)
	return append(dst, '}'), nil
}

func (recv AnyHolder) DecodeFrom(data []byte) (result AnyHolder, i int, err error) {
	result = recv
	seenData := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "data":
			if seenData {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"data"}}
			}
			seenData = true
			result.Data, i, err = scan.Any(data, i)
			if err != nil {
				return result, i, decode.NewParseErr("data", i, err)
			}
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv AnyHolder) DecodeFromStream(s *scan.Stream) (result AnyHolder, err error) {
	result = recv
	seenData := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "data":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("data", s.Offset(), err)
			}
			if seenData {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"data"}}
			}
			seenData = true
			result.Data, err = s.Any()
			if err != nil {
				return result, decode.NewParseErr("data", s.Offset(), err)
			}
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s AnyHolder) JSONSize() int {
	size := 265
	return size
}

func (s AnyHolder) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"data\":"...)
	if dst, err = encode.AppendAny(dst, s.Data); err != nil {
		return dst, err
	}
	return append(dst, '}'), nil
}

func (recv RawHolder) DecodeFrom(data []byte) (result RawHolder, i int, err error) {
	result = recv
	seenRaw := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "raw":
			if seenRaw {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"raw"}}
			}
			seenRaw = true
			start := i
			i, err = scan.SkipValue(data, start)
			if err != nil {
				return result, i, decode.NewParseErr("raw", i, err)
			}
			err = scan.CheckUTF8(data[start:i])
			if err != nil {
				return result, i, decode.NewParseErr("raw", i, err)
			}
			result.Raw = data[start:i]
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv RawHolder) DecodeFromStream(s *scan.Stream) (result RawHolder, err error) {
	result = recv
	seenRaw := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "raw":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("raw", s.Offset(), err)
			}
			if seenRaw {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"raw"}}
			}
			seenRaw = true
			span, err := s.CaptureValue()
			if err != nil {
				return result, decode.NewParseErr("raw", s.Offset(), err)
			}
			err = scan.CheckUTF8(span)
			if err != nil {
				return result, decode.NewParseErr("raw", s.Offset(), err)
			}
			result.Raw = append(make([]byte, 0, len(span)), span...)
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s RawHolder) JSONSize() int {
	size := 8
	if n := len(s.Raw); n > 0 {
		size += n
	} else {
		size += 4
	}
	return size
}

func (s RawHolder) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"raw\":"...)
	if len(s.Raw) == 0 {
		dst = append(dst, "null"...)
	} else {
		dst = append(dst, s.Raw...)
	}
	return append(dst, '}'), nil
}

func (recv NumGrammar) DecodeFrom(data []byte) (result NumGrammar, i int, err error) {
	result = recv
	seenF := false
	seenI := false
	seenN := false
	seenU := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "f":
			if seenF {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"f"}}
			}
			seenF = true
			result.F, i, err = scan.Float64(data, i)
			if err != nil {
				return result, i, decode.NewParseErr("f", i, err)
			}
		case "i":
			if seenI {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"i"}}
			}
			seenI = true
			neg := false
			if i < len(data) && data[i] == '-' {
				neg = true
				i++
			}
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("i", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("i", i, scan.ErrBadNumber)
			}
			limit := uint64(math.MaxInt64)
			if neg {
				limit = scan.SignedNeg
			}
			var u uint64
			de := i + 18
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				u = u*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if u > limit/10 || (u == limit/10 && d > limit%10) {
					return result, i, decode.NewParseErr("i", i, scan.ErrNumberOverflow)
				}
				u = u*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("i", i, scan.ErrBadNumber)
				}
			}
			var n int64
			if neg {
				if u == scan.SignedNeg {
					n = math.MinInt64
				} else {
					n = -int64(u)
				}
			} else {
				n = int64(u)
			}
			result.I = int(n)
		case "n":
			if seenN {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"n"}}
			}
			seenN = true
			start := i
			i, err = scan.SkipValue(data, start)
			if err != nil {
				return result, i, decode.NewParseErr("n", i, err)
			}
			err = json.Unmarshal(data[start:i], &result.N)
			if err != nil {
				return result, i, decode.NewParseErr("n", i, err)
			}
		case "u":
			if seenU {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"u"}}
			}
			seenU = true
			if i >= len(data) || data[i] < '0' || data[i] > '9' {
				return result, i, decode.NewParseErr("u", i, scan.ErrBadNumber)
			}
			if data[i] == '0' && i+1 < len(data) && data[i+1] >= '0' && data[i+1] <= '9' {
				return result, i, decode.NewParseErr("u", i, scan.ErrBadNumber)
			}
			var n uint64
			de := i + 19
			if de > len(data) {
				de = len(data)
			}
			for i < de && data[i] >= '0' && data[i] <= '9' {
				n = n*10 + uint64(data[i]-'0')
				i++
			}
			for i < len(data) && data[i] >= '0' && data[i] <= '9' {
				d := uint64(data[i] - '0')
				if n > scan.Uint64Limit/10 || (n == scan.Uint64Limit/10 && d > scan.Uint64Limit%10) {
					return result, i, decode.NewParseErr("u", i, scan.ErrNumberOverflow)
				}
				n = n*10 + d
				i++
			}
			if i < len(data) {
				c := data[i]
				if c == '.' || c == 'e' || c == 'E' {
					return result, i, decode.NewParseErr("u", i, scan.ErrBadNumber)
				}
			}
			result.U = uint(n)
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv NumGrammar) DecodeFromStream(s *scan.Stream) (result NumGrammar, err error) {
	result = recv
	seenF := false
	seenI := false
	seenN := false
	seenU := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "f":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("f", s.Offset(), err)
			}
			if seenF {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"f"}}
			}
			seenF = true
			result.F, err = s.Float64()
			if err != nil {
				return result, decode.NewParseErr("f", s.Offset(), err)
			}
		case "i":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("i", s.Offset(), err)
			}
			if seenI {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"i"}}
			}
			seenI = true
			var iv int64
			iv, err = s.Int64()
			if err != nil {
				return result, decode.NewParseErr("i", s.Offset(), err)
			}
			result.I = int(iv)
		case "n":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("n", s.Offset(), err)
			}
			if seenN {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"n"}}
			}
			seenN = true
			span, err := s.CaptureValue()
			if err != nil {
				return result, decode.NewParseErr("n", s.Offset(), err)
			}
			err = json.Unmarshal(span, &result.N)
			if err != nil {
				return result, decode.NewParseErr("n", s.Offset(), err)
			}
		case "u":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("u", s.Offset(), err)
			}
			if seenU {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"u"}}
			}
			seenU = true
			var uv uint64
			uv, err = s.Uint64()
			if err != nil {
				return result, decode.NewParseErr("u", s.Offset(), err)
			}
			result.U = uint(uv)
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s NumGrammar) JSONSize() int {
	size := 214
	return size
}

func (s NumGrammar) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"f\":"...)
	if dst, err = encode.AppendFloat(dst, s.F, 64); err != nil {
		return dst, err
	}
	dst = append(dst, ",\"i\":"...)
	dst = strconv.AppendInt(dst, int64(s.I), 10)
	dst = append(dst, ",\"n\":"...)
	var bN []byte
	bN, err = json.Marshal(s.N)
	if err != nil {
		return dst, err
	}
	dst = append(dst, bN...)
	dst = append(dst, ",\"u\":"...)
	dst = strconv.AppendUint(dst, uint64(s.U), 10)
	return append(dst, '}'), nil
}

func (recv NarrowFloats) DecodeFrom(data []byte) (result NarrowFloats, i int, err error) {
	result = recv
	if result.Fm != nil {
		clear(result.Fm)
	}
	if result.Fs != nil {
		result.Fs = result.Fs[:0]
	}
	seenF := false
	seenFm := false
	seenFp := false
	seenFs := false
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i >= len(data) || data[i] != '{' {
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
	i++
	for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
		i++
	}
	if i < len(data) && data[i] == '}' {
		i++
		return result, i, nil
	}
	for {
		var key string
		if i >= len(data) || data[i] != '"' {
			return result, i, decode.NewParseErr("", i, scan.ErrExpectString)
		}
		ke := i + 1
		for ke < len(data) && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
			ke++
		}
		if ke < len(data) && data[ke] == '"' {
			key = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
			i = ke + 1
		} else {
			key, i, err = scan.String(data, i, true)
			if err != nil {
				return result, i, decode.NewParseErr("", i, err)
			}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) || data[i] != ':' {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		i++
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		switch key {
		case "f":
			if seenF {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"f"}}
			}
			seenF = true
			var fv float64
			fv, i, err = scan.Float64(data, i)
			if err != nil {
				return result, i, decode.NewParseErr("f", i, err)
			}
			if math.IsInf(float64(float32(fv)), 0) {
				return result, i, decode.NewParseErr("f", i, scan.ErrNumberOverflow)
			}
			result.F = float32(fv)
		case "fm":
			if seenFm {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"fm"}}
			}
			seenFm = true
			if i+4 <= len(data) && data[i] == 'n' && data[i+1] == 'u' && data[i+2] == 'l' && data[i+3] == 'l' {
				i += 4
				result.Fm = nil
				break
			}
			if i >= len(data) || data[i] != '{' {
				return result, i, decode.NewParseErr("fm", i, scan.ErrBadObject)
			}
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			if i < len(data) && data[i] == '}' {
				if result.Fm == nil {
					result.Fm = map[string]float32{}
				}
			} else {
				if result.Fm == nil {
					result.Fm = make(map[string]float32)
				}
			}
			if i < len(data) && data[i] != '}' {
				for {
					var mk string
					if i >= len(data) || data[i] != '"' {
						return result, i, decode.NewParseErr("fm", i, scan.ErrExpectString)
					}
					ke := i + 1
					kew := ke + 32
					if kew > len(data) {
						kew = len(data)
					}
					for ke < kew && data[ke] != '"' && data[ke] != '\\' && data[ke] >= 0x20 && data[ke] < 0x80 {
						ke++
					}
					if ke < len(data) && data[ke] == '"' {
						mk = unsafe.String(unsafe.SliceData(data[i+1:]), ke-i-1)
						i = ke + 1
					} else {
						mk, i, err = scan.String(data, i, true)
						if err != nil {
							return result, i, decode.NewParseErr("fm", i, err)
						}
					}
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i >= len(data) || data[i] != ':' {
						return result, i, decode.NewParseErr("fm", i, scan.ErrBadObject)
					}
					i++
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					var fv float64
					fv, i, err = scan.Float64(data, i)
					if err != nil {
						return result, i, decode.NewParseErr("fm", i, err)
					}
					if math.IsInf(float64(float32(fv)), 0) {
						return result, i, decode.NewParseErr("fm", i, scan.ErrNumberOverflow)
					}
					result.Fm[mk] = float32(fv)
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i < len(data) && data[i] == ',' {
						i++
						for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
							i++
						}
						if i >= len(data) || data[i] == '}' {
							return result, i, decode.NewParseErr("fm", i, scan.ErrBadObject)
						}
						continue
					}
					break
				}
			}
			if i >= len(data) || data[i] != '}' {
				return result, i, decode.NewParseErr("fm", i, scan.ErrBadObject)
			}
			i++
		case "fp":
			if seenFp {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"fp"}}
			}
			seenFp = true
			if i+4 <= len(data) && data[i] == 'n' && data[i+1] == 'u' && data[i+2] == 'l' && data[i+3] == 'l' {
				i += 4
				result.Fp = nil
				break
			}
			var v float64
			v, i, err = scan.Float64(data, i)
			if err != nil {
				return result, i, decode.NewParseErr("fp", i, err)
			}
			if math.IsInf(float64(float32(v)), 0) {
				return result, i, decode.NewParseErr("fp", i, scan.ErrNumberOverflow)
			}
			if result.Fp == nil {
				result.Fp = new(float32(v))
			} else {
				(*result.Fp) = float32(v)
			}
		case "fs":
			if seenFs {
				return result, i, &validation.DuplicateKeyError{Pos: i, Path: []string{"fs"}}
			}
			seenFs = true
			if i+4 <= len(data) && data[i] == 'n' && data[i+1] == 'u' && data[i+2] == 'l' && data[i+3] == 'l' {
				i += 4
				result.Fs = nil
				break
			}
			if i >= len(data) || data[i] != '[' {
				return result, i, decode.NewParseErr("fs", i, scan.ErrBadArray)
			}
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			if i < len(data) && data[i] == ']' {
				if result.Fs == nil {
					result.Fs = []float32{}
				}
			} else {
				if result.Fs == nil {
					cnt0 := 1
					if e := bytes.IndexByte(data[i:], ']'); e >= 0 {
						cnt0 = bytes.Count(data[i:i+e], []byte{','}) + 1
					}
					result.Fs = make([]float32, 0, cnt0)
				}
			}
			if i < len(data) && data[i] != ']' {
				for {
					result.Fs = append(result.Fs, 0)
					var fv float64
					fv, i, err = scan.Float64(data, i)
					if err != nil {
						return result, i, decode.NewParseErr("fs", i, err)
					}
					if math.IsInf(float64(float32(fv)), 0) {
						return result, i, decode.NewParseErr("fs", i, scan.ErrNumberOverflow)
					}
					result.Fs[len(result.Fs)-1] = float32(fv)
					for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
						i++
					}
					if i < len(data) && data[i] == ',' {
						i++
						for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
							i++
						}
						if i >= len(data) || data[i] == ']' {
							return result, i, decode.NewParseErr("fs", i, scan.ErrBadArray)
						}
						continue
					}
					break
				}
			}
			if i >= len(data) || data[i] != ']' {
				return result, i, decode.NewParseErr("fs", i, scan.ErrBadArray)
			}
			i++
		default:
			return result, i, &validation.UnknownKeyError{Pos: i, Path: []string{key}}
		}
		for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
			i++
		}
		if i >= len(data) {
			return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
		}
		if data[i] == ',' {
			i++
			for i < len(data) && data[i] <= ' ' && (data[i] == ' ' || data[i] == '\t' || data[i] == '\n' || data[i] == '\r') {
				i++
			}
			continue
		}
		if data[i] == '}' {
			i++
			return result, i, nil
		}
		return result, i, decode.NewParseErr("", i, scan.ErrBadObject)
	}
}

func (recv NarrowFloats) DecodeFromStream(s *scan.Stream) (result NarrowFloats, err error) {
	result = recv
	if result.Fm != nil {
		clear(result.Fm)
	}
	if result.Fs != nil {
		result.Fs = result.Fs[:0]
	}
	seenF := false
	seenFm := false
	seenFp := false
	seenFs := false
	err = s.ObjectOpen()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	err = s.SkipSpace()
	if err != nil {
		return result, decode.NewParseErr("", s.Offset(), err)
	}
	if s.Pos >= len(s.Bytes()) {
		if err = s.ReadMore(s.Pos); err != nil {
			return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrExpectString))
		}
		s.Pos = 0
	}
	if s.Bytes()[s.Pos] == '}' {
		s.Pos++
		return result, nil
	}
	for {
		var key string
		key, err = s.KeyView(true)
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		switch key {
		case "f":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("f", s.Offset(), err)
			}
			if seenF {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"f"}}
			}
			seenF = true
			var fv float64
			fv, err = s.Float64()
			if err != nil {
				return result, decode.NewParseErr("f", s.Offset(), err)
			}
			if math.IsInf(float64(float32(fv)), 0) {
				return result, decode.NewParseErr("f", s.Offset(), scan.ErrNumberOverflow)
			}
			result.F = float32(fv)
		case "fm":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("fm", s.Offset(), err)
			}
			if seenFm {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"fm"}}
			}
			seenFm = true
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("fm", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
				}
			}
			if s.Bytes()[s.Pos] == 'n' {
				for ki := 1; ki < 4; ki++ {
					if s.Pos+ki >= len(s.Bytes()) {
						if err = s.ReadMore(0); err != nil {
							return result, decode.NewParseErr("fm", s.Offset(), scan.NotEOF(err, scan.ErrBadLiteral))
						}
					}
					if s.Bytes()[s.Pos+ki] != "null"[ki] {
						return result, decode.NewParseErr("fm", s.Offset(), scan.ErrBadLiteral)
					}
				}
				s.Pos += 4
				result.Fm = nil
				break
			}
			err = s.ObjectOpen()
			if err != nil {
				return result, decode.NewParseErr("fm", s.Offset(), err)
			}
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("fm", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
				}
			}
			if s.Bytes()[s.Pos] == '}' {
				if result.Fm == nil {
					result.Fm = map[string]float32{}
				}
			} else {
				if result.Fm == nil {
					result.Fm = make(map[string]float32)
				}
			}
			for s.Bytes()[s.Pos] != '}' {
				var mk string
				mk, err = s.String(true)
				if err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), err)
				}
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), err)
				}
				if s.Pos >= len(s.Bytes()) {
					if err = s.ReadMore(0); err != nil {
						return result, decode.NewParseErr("fm", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
					}
				}
				if s.Bytes()[s.Pos] != ':' {
					return result, decode.NewParseErr("fm", s.Offset(), scan.ErrBadObject)
				}
				s.Pos++
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), err)
				}
				var fv float64
				fv, err = s.Float64()
				if err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), err)
				}
				if math.IsInf(float64(float32(fv)), 0) {
					return result, decode.NewParseErr("fm", s.Offset(), scan.ErrNumberOverflow)
				}
				result.Fm[mk] = float32(fv)
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("fm", s.Offset(), err)
				}
				if s.Pos >= len(s.Bytes()) {
					if err = s.ReadMore(0); err != nil {
						return result, decode.NewParseErr("fm", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
					}
				}
				if s.Bytes()[s.Pos] == ',' {
					s.Pos++
					err = s.SkipSpace()
					if err != nil {
						return result, decode.NewParseErr("fm", s.Offset(), err)
					}
					if s.Pos >= len(s.Bytes()) || s.Bytes()[s.Pos] == '}' {
						return result, decode.NewParseErr("fm", s.Offset(), scan.ErrBadObject)
					}
					continue
				}
				break
			}
			if s.Bytes()[s.Pos] != '}' {
				return result, decode.NewParseErr("fm", s.Offset(), scan.ErrBadObject)
			}
			s.Pos++
		case "fp":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("fp", s.Offset(), err)
			}
			if seenFp {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"fp"}}
			}
			seenFp = true
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("fp", s.Offset(), scan.NotEOF(err, scan.ErrBadNumber))
				}
			}
			if s.Bytes()[s.Pos] == 'n' {
				for ki := 1; ki < 4; ki++ {
					if s.Pos+ki >= len(s.Bytes()) {
						if err = s.ReadMore(0); err != nil {
							return result, decode.NewParseErr("fp", s.Offset(), scan.NotEOF(err, scan.ErrBadLiteral))
						}
					}
					if s.Bytes()[s.Pos+ki] != "null"[ki] {
						return result, decode.NewParseErr("fp", s.Offset(), scan.ErrBadLiteral)
					}
				}
				s.Pos += 4
				result.Fp = nil
				break
			}
			var v float64
			v, err = s.Float64()
			if err != nil {
				return result, decode.NewParseErr("fp", s.Offset(), err)
			}
			if math.IsInf(float64(float32(v)), 0) {
				return result, decode.NewParseErr("fp", s.Offset(), scan.ErrNumberOverflow)
			}
			if result.Fp == nil {
				result.Fp = new(float32(v))
			} else {
				(*result.Fp) = float32(v)
			}
		case "fs":
			err = s.ConsumeColon()
			if err != nil {
				return result, decode.NewParseErr("fs", s.Offset(), err)
			}
			if seenFs {
				return result, &validation.DuplicateKeyError{Pos: s.Offset(), Path: []string{"fs"}}
			}
			seenFs = true
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("fs", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("fs", s.Offset(), scan.NotEOF(err, scan.ErrBadArray))
				}
			}
			if s.Bytes()[s.Pos] == 'n' {
				for ki := 1; ki < 4; ki++ {
					if s.Pos+ki >= len(s.Bytes()) {
						if err = s.ReadMore(0); err != nil {
							return result, decode.NewParseErr("fs", s.Offset(), scan.NotEOF(err, scan.ErrBadLiteral))
						}
					}
					if s.Bytes()[s.Pos+ki] != "null"[ki] {
						return result, decode.NewParseErr("fs", s.Offset(), scan.ErrBadLiteral)
					}
				}
				s.Pos += 4
				result.Fs = nil
				break
			}
			err = s.ArrayOpen()
			if err != nil {
				return result, decode.NewParseErr("fs", s.Offset(), err)
			}
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("fs", s.Offset(), err)
			}
			if s.Pos >= len(s.Bytes()) {
				if err = s.ReadMore(0); err != nil {
					return result, decode.NewParseErr("fs", s.Offset(), scan.NotEOF(err, scan.ErrBadArray))
				}
			}
			if s.Bytes()[s.Pos] == ']' {
				if result.Fs == nil {
					result.Fs = []float32{}
				}
			} else {
				if result.Fs == nil {
					result.Fs = make([]float32, 0, ggenCap_NarrowFloats_Fs_float32)
				}
			}
			for s.Bytes()[s.Pos] != ']' {
				result.Fs = append(result.Fs, 0)
				var fv float64
				fv, err = s.Float64()
				if err != nil {
					return result, decode.NewParseErr("fs", s.Offset(), err)
				}
				if math.IsInf(float64(float32(fv)), 0) {
					return result, decode.NewParseErr("fs", s.Offset(), scan.ErrNumberOverflow)
				}
				result.Fs[len(result.Fs)-1] = float32(fv)
				err = s.SkipSpace()
				if err != nil {
					return result, decode.NewParseErr("fs", s.Offset(), err)
				}
				if s.Pos >= len(s.Bytes()) {
					if err = s.ReadMore(0); err != nil {
						return result, decode.NewParseErr("fs", s.Offset(), scan.NotEOF(err, scan.ErrBadArray))
					}
				}
				if s.Bytes()[s.Pos] == ',' {
					s.Pos++
					err = s.SkipSpace()
					if err != nil {
						return result, decode.NewParseErr("fs", s.Offset(), err)
					}
					if s.Pos >= len(s.Bytes()) || s.Bytes()[s.Pos] == ']' {
						return result, decode.NewParseErr("fs", s.Offset(), scan.ErrBadArray)
					}
					continue
				}
				break
			}
			if s.Bytes()[s.Pos] != ']' {
				return result, decode.NewParseErr("fs", s.Offset(), scan.ErrBadArray)
			}
			s.Pos++
		default:
			return result, &validation.UnknownKeyError{Pos: s.Offset(), Path: []string{strings.Clone(key)}}
		}

		err = s.SkipSpace()
		if err != nil {
			return result, decode.NewParseErr("", s.Offset(), err)
		}
		if s.Pos >= len(s.Bytes()) {
			if err = s.ReadMore(s.Pos); err != nil {
				return result, decode.NewParseErr("", s.Offset(), scan.NotEOF(err, scan.ErrBadObject))
			}
			s.Pos = 0
		}
		c := s.Bytes()[s.Pos]
		if c == ',' {
			s.Pos++
			err = s.SkipSpace()
			if err != nil {
				return result, decode.NewParseErr("", s.Offset(), err)
			}
			continue
		}
		if c == '}' {
			s.Pos++
			return result, nil
		}
		return result, decode.NewParseErr("", s.Offset(), scan.ErrBadObject)
	}
}

func (s NarrowFloats) JSONSize() int {
	size := 57
	size += len(s.Fm) * 29
	for k := range s.Fm {
		size += len(k) * 2
	}
	if s.Fp == nil {
		size += 4
	} else {
		size += 25
	}
	if n := len(s.Fs); n > 0 {
		size += n - 1
	}
	size += len(s.Fs) * 25
	return size
}

func (s NarrowFloats) AppendJSON(dst []byte) ([]byte, error) {
	var err error
	_ = err
	dst = append(dst, "{\"f\":"...)
	if dst, err = encode.AppendFloat(dst, float64(s.F), 32); err != nil {
		return dst, err
	}
	dst = append(dst, ",\"fm\":"...)
	if s.Fm == nil {
		dst = append(dst, "null"...)
	} else {
		dst = append(dst, '{')
		firstFm := true
		for k, v := range s.Fm {
			if firstFm {
				firstFm = false
				dst = append(dst, '"')
			} else {
				dst = append(dst, ",\""...)
			}
			dst = encode.AppendStringNoHTML(dst, k)
			dst = append(dst, ':')
			if dst, err = encode.AppendFloat(dst, float64(v), 32); err != nil {
				return dst, err
			}
		}
		dst = append(dst, '}')
	}
	dst = append(dst, ",\"fp\":"...)
	if s.Fp == nil {
		dst = append(dst, "null"...)
	} else {
		if dst, err = encode.AppendFloat(dst, float64((*s.Fp)), 32); err != nil {
			return dst, err
		}
	}
	dst = append(dst, ",\"fs\":"...)
	if s.Fs == nil {
		dst = append(dst, "null"...)
	} else {
		dst = append(dst, '[')
		if len(s.Fs) > 0 {
			if dst, err = encode.AppendFloat(dst, float64(s.Fs[0]), 32); err != nil {
				return dst, err
			}
			for _, v0 := range s.Fs[1:] {
				dst = append(dst, ',')
				if dst, err = encode.AppendFloat(dst, float64(v0), 32); err != nil {
					return dst, err
				}
			}
		}
		dst = append(dst, ']')
	}
	return append(dst, '}'), nil
}
