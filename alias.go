...
// usesUnsafe reports whether any generated code in the pass needs the
// `unsafe` import. Triggered by:
//   - struct decoders (inline key extract via `unsafe.String`)
//   - struct-alias delegation through TextUnmarshaler (we hand the
//     decoded JSON string to UnmarshalText via `unsafe.Slice` so the
//     underlying method gets bytes without an intermediate copy)
func usesUnsafe(structs []StructInfo) bool {
	for _, s := range structs {
		if !s.IsAlias {
			return true
		}
		if s.AliasKind == KindStruct && aliasUsesTextDecode(s) {
			return true
		}
	}
	return false
...
// aliasUsesTextDecode reports whether the struct-alias's chosen decode
// path lands on TextUnmarshaler. Mirrors the precedence ladder in
// renderAliasStructDecode (ByteDecoder > StreamDecoder > JSONUnmarshaler
// > TextUnmarshaler).
func aliasUsesTextDecode(s StructInfo) bool {
	if s.AliasIface.ByteDecoder || s.AliasIface.StreamDecoder {
		return false
	}
	if s.AliasIface.JSONUnmarshaler {
		return false
	}
	return s.AliasIface.TextUnmarshaler
}

// usesStrconv reports whether the pass needs `strconv`. Walks every
// struct field (and aliases) checking whether the emitted code will
// actually call into strconv. Strings, raw-JSON, URL, net.IP, base64
// bytes, and nested struct/alias method-calls do NOT use strconv;
// numerics, bools, time/duration, sql.Null* with a numeric inner, and
// `json:",string"`-wrapped primitives DO.
func usesStrconv(structs []StructInfo) bool {
	for _, s := range structs {
		if s.IsAlias {
			if scalarKindNeedsStrconv(s.AliasKind) {
				return true
			}
			continue
		}
		if slices.ContainsFunc(s.Fields, fieldNeedsStrconv) {
			return true
		}
	}
	return false
...
// scalarKindNeedsStrconv classifies primitive TypeKinds that emit
// strconv.AppendBool / AppendInt / AppendUint / AppendFloat or their
// strconv.Parse* counterparts on decode-via-string-tag.
func scalarKindNeedsStrconv(k TypeKind) bool {
	switch k {
	case KindBool,
...
// fieldNeedsStrconv covers the field-level cases beyond plain scalar
// kinds: time/duration unix-format paths, sql.Null* wrappers around
// numerics, `json:",string"` opt-in, and slice/map/array element kinds.
func fieldNeedsStrconv(f FieldInfo) bool {
	if scalarKindNeedsStrconv(f.Kind) {
		return true
	}
	if f.String {
		return true
	...
	case KindTime, KindDuration:
		return true
	case KindSQLNull:
		if spec, ok := SQLNullSpec(f.GoType); ok && scalarKindNeedsStrconv(spec.Inner) {
			return true
		}
	case KindSlice, KindArray, KindMap:
		if scalarKindNeedsStrconv(f.ElemKind) {
			return true
		}
	}
	return false
