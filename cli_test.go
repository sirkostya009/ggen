...
`)
	...
	// --- -noformat opt-out ---
	//
	// Default path runs the rendered output through go/format.Source,
	// producing fully gofmt-clean code. -noformat ships the renderer
	// output verbatim: still structurally valid Go, but single-tab
	// indentation (the renderers don't track nesting depth).

	t.Run("Noformat_DefaultIsGofmtClean", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		writeFixture(t, filepath.Join(dir, "msg.go"), minimalStruct)
		if out, err := runCLI(t, bin, dir, "msg.go"); err != nil {
		...
		// first line. Look for `\n\tvar err error\n` — gofmt indents
		// it; the unformatted renderer leaves it flush-left.
		if !strings.Contains(body, "\n\tvar err error\n") {
			t.Errorf("default output not gofmt-clean (var err error not indented):\n%s", body)
		}
	})

	t.Run("Noformat_SkipsFormat", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		writeFixture(t, filepath.Join(dir, "msg.go"), minimalStruct)
		if out, err := runCLI(t, bin, dir, "-noformat", "msg.go"); err != nil {
			t.Fatalf("ggen -noformat: %v\n%s", err, out)
		}
		body := mustReadOutput(t, filepath.Join(dir, "msg_ggen.go"))
		// Unformatted: renderer output starts at column 0 from the
		// {{decode .}} expansion point. Only the FIRST line picks up
		// the template's leading tab; subsequent lines (e.g.
		// `var err error`) sit flush-left.
		if !strings.Contains(body, "\nvar err error\n") {
			t.Errorf("expected flush-left renderer output under -noformat, got:\n%s", body)
		}
		// Sanity-check the structure is otherwise intact.
		for _, want := range []string{
			"// Code generated by ggen; DO NOT EDIT.",
			"package fixture",
			"func (Msg) DecodeFrom",
			"func (s Msg) AppendJSON",
		} {
			if !strings.Contains(body, want) {
				t.Errorf("missing %q in -noformat output:\n%s", want, body)
			}
		}
	})

	t.Run("Noformat_OutputStillCompiles", func(t *testing.T) {
		t.Parallel()
		dir := t.TempDir()
		writeFixture(t, filepath.Join(dir, "msg.go"), minimalStruct)
		if out, err := runCLI(t, bin, dir, "-noformat", "msg.go"); err != nil {
		...
		// Round-trip through go/format to confirm the unformatted output
		// is still parseable Go. format.Source errors on syntactically
		// invalid input.
		body := mustReadOutput(t, filepath.Join(dir, "msg_ggen.go"))
		if _, err := format.Source([]byte(body)); err != nil {
			t.Errorf("-noformat output failed format.Source (not valid Go):\n%v\n\nbody:\n%s", err, body)
		}
	})
