...
- `[0]T` — marshals as `[]`, decodes only `[]` (anything else → `ggen.LenError{Want: 0, Got: 1}`), in every position: field, slice element, array slot, map value, nested `[0][N]T`.
...
- Embedded struct — fields promoted to parent JSON object. Two promoted fields sharing a Go name but not a JSON name = generate-time error (stdlib keeps both).
